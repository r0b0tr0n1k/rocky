...
```txt
Domain Data (DB → TypeScript types)
    ↓  (DocumentTemplate.fetchData + mapToModel)
Document Model (versioned, conforms to models/*.yaml)
    ↓  (YamlSerializer)
YAML/XML Output (archivable intermediate format)
    ↓  (PDF Engine — deferred, pluggable)
PDF / PDF/A (environment-specific rendering)
```
