...
export const modules = pgTable("modules", {
  ...
  type: moduleTypePgEnum("type").notNull(),
...
export const businessRules = pgTable(
  ...
  (table) => [uniqueIndex("idx_br_name").on(table.name)],
);
...
export const codeTables = pgTable(
  ...
  (table) => [
    uniqueIndex("idx_code_tables_group_code").on(table.group, table.code),
    index("idx_code_tables_group").on(table.group),
  ],
);
...
export const systemParameters = pgTable(
  ...
  (table) => [uniqueIndex("idx_sys_params_code").on(table.code), index("idx_sys_params_group").on(table.group)],
);
