...
CREATE POLICY "lab_test_access_policy" ON "lab_tests" AS PERMISSIVE FOR ALL TO public USING ((
    ...
    OR (current_setting('app.current_role', true) = ANY(($6, $7, $8, $9)) AND "lab_tests"."farm_id" IN (
...
CREATE POLICY "vaccine_disease_access_policy" ON "vaccine_diseases" AS PERMISSIVE FOR ALL TO public USING (current_setting('app.current_role', true) = ANY(($1, $2, $3))) WITH CHECK (current_setting('app.current_role', true) = ANY(($1, $2, $3)));