...
CREATE POLICY "treatment_access_policy" ON "treatments" AS PERMISSIVE FOR ALL TO public USING ((
    ...
    OR (current_setting('app.current_role', true) = ANY(ARRAY['FARMER', 'SLAUGHTERHOUSE_OP', 'MARKET_OP', 'SUPPLIER']) AND "treatments"."farm_id" IN (
...
CREATE POLICY "vaccination_access_policy" ON "vaccinations" AS PERMISSIVE FOR ALL TO public USING ((
    ...
    OR (current_setting('app.current_role', true) = ANY(ARRAY['FARMER', 'SLAUGHTERHOUSE_OP', 'MARKET_OP', 'SUPPLIER']) AND "vaccinations"."farm_id" IN (
    SELECT fs.farm_id FROM farm_subjects fs
    WHERE fs.subject_id = current_setting('app.current_user_id', true)::uuid
  ))
  )) WITH CHECK (current_setting('app.current_role', true) = ANY(ARRAY['SUPER_ADMIN', 'VD_ADMIN', 'VD_STAFF', 'VETERINARIAN', 'TECHNICIAN']));--> statement-breakpoint
...
CREATE POLICY "vaccine_access_policy" ON "vaccines" AS PERMISSIVE FOR ALL TO public USING (current_setting('app.current_role', true) = ANY(ARRAY['SUPER_ADMIN', 'VD_ADMIN', 'VD_STAFF'])) WITH CHECK (current_setting('app.current_role', true) = ANY(ARRAY['SUPER_ADMIN', 'VD_ADMIN', 'VD_STAFF']));