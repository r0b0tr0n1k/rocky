...
> **Canonical references:** See `docs/VALIDATOR_DESIGN_GUIDE.md` (how-to), **ADR 0010** (two-layer date coercion: L2 `z.coerce.date()` tolerant, L1 `z.coerce.date<string>()` strict), and **ADR 0018** (ratified standard: schema derivation, `.pick()` over `.omit()`, `.strict()` target, three-tier guillotine).
...
### 2.5 Enums — The Dialectic of the Barrel

**Never use inline unions.** Always import from the enum SSOT.

DB-backed enums are defined **once** as atoms (Dictionary + `VALUES` array) in `packages/database/src/constants/` and re-exported as readonly constants. From those constants, `packages/validators/src/enums/domain.ts` auto-generates **branded Zod schemas** (`*Schema`) and `index.ts` re-exports them alongside the curated dictionaries. The API/event layer only ever *consumes* — it never defines enum atoms (doing so is contamination → the schema becomes Tier 3).

Two distinct imports, for two distinct purposes:

| Import                      | What you get                                                | Use it for                                                                                              |
| --------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `../enums/index.js`         | branded Zod enum schemas (`*Schema`) + curated dictionaries | validation inside a schema, e.g. `.extend({ status: statusSchema })`, or `z.infer<typeof statusSchema>` |
| `@rocky/database/constants` | plain readonly dictionaries (`STATUS`)                      | runtime lookups, switches, and passing the `VALUES` array into `z.enum()` ONLY when no schema exists    |

**Always import the schema/dictionary form from `../enums/index.js`, never `../enums/domain.js`.**

```typescript
// ✅ Correct — public barrel
...
// ❌ Crime — hand-written union, drifts from the DB constant
const statusSchema = z.enum(["alive", "dead", "slaughtered", "stolen", "lost", "exported", "imported", "birth"]);
```

The `index.ts` barrel is the **Symbolic Order** — the curated public surface. `domain.ts` is the auto-generated implementation carrying its own `NoDrift` proofs and regenerated by `scripts/regenerate-enums.mjs`; binding `api/`/`events/` files to it couples your contract to generator internals and enables four failures:

1. **Import-boundary violation.** The `api/` allowlist permits `../enums` (the barrel). A deep import into `../enums/domain.js` crosses into an internal generator artifact and trips the architecture rules — the build fails.
2. **Lost curated dictionaries.** `index.js` re-exports the uppercase `CONSTANT` dictionaries alongside the schemas; `domain.js` only exposes schemas and raw `*_VALUES`. Importing `domain.js` tempts you to pull `*_VALUES` straight from the generator instead of the SSOT dictionary.
3. **Footgun: hand-built `z.enum`.** Because `domain.js` makes the `*_VALUES` arrays visible, it is easy to write `z.enum(SOME_STATUS_VALUES)` by hand — reintroducing exactly the drift risk the branded `*Schema` was generated to prevent. `index.js` hides the atoms so you can only grab the finished schema.
4. **Coupling to generation internals.** A restructure of the generated file breaks every deep importer at once.

Rule of thumb: if you need it *inside a Zod schema or as a TypeScript type*, import the `*Schema` from `../enums/index.js`. If you need the raw string union *at runtime* (a switch, an array of options), import the `CONSTANT` from `@rocky/database/constants`. Never write `z.enum([...])` by hand in `api/`/`events/` — values must be referenced from the SSOT constants so they can never diverge.

The enum chain is: `database/constants/` → `validators/enums/domain.ts` (generator) → `zEnum()` → `validators/enums/index.ts` (barrel) → exported as `*Schema` + `*Type` + `*_VALUES` dictionary.
...
- `../enums/index.js` — the public barrel: enum schemas used in payloads (never `domain.js`)
...
- [ ] Query params / special inputs: `z.strictObject({...})`
  - [ ] Enums: from `../enums/index.js`, never inline, never `domain.js`
...
│   ├── health.events.ts       ← Health lifecycle events
│   └── index.ts               ← Barrel
├── enums/                     ← zEnum schemas from database constants
│   ├── domain.ts              ← PRIVATE: auto-generated zEnum schemas + NoDrift guillotines (do NOT import)
│   └── index.ts               ← PUBLIC barrel: re-exports dictionaries + schemas + types (import this)
...
## 10. Consumer Topology — Who Consumes These Validators

The `api/[domain].api.ts` schemas are the **single contract** for every external caller. The validators package is a **vertical waterfall**, not a horizontal utility: each consumer sits at a fixed layer and sees only the shadow of the layer above it. Horizontal bleeding of types is forbidden.

### 10.1 API schema consumers

| Consumer                             | Imports                                         | Rule                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------ | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apps/api` tRPC routers              | `*Schema` from `@rocky/validators/api/index.js` | Primary consumer. Every router uses the schema as its input/response type (the schema **is** the tRPC procedure contract). Routers do NOT import Dumb Zod or enums directly — all shape arrives via api.ts. Codegen is driven by `nestjs-trpc generate --entrypoint src/app.module.ts` (the runner scans the router files, which already pull schemas from `@rocky/validators/api`); `apps/api/src/trpc/trpc.module.ts` only configures `TRPCModule.forRoot` — it does not barrel-import api schemas. |
| `apps/web`, `apps/expo` (frontends)  | **NOT** api schemas directly                    | They consume the **generated tRPC client** (`@rocky/trpc` → `packages/trpc/src/generated/server.ts`), whose types are derived from these schemas. The border stays server-side.                                                                                                                                                                                                                                                                                                                       |
| `apps/worker`                        | **NOT** api schemas                             | Workers consume `events/` schemas (the event protocol), not the API gate. Keep this boundary strict.                                                                                                                                                                                                                                                                                                                                                                                                  |
| `packages/domains` (domain services) | **NOT** api schemas                             | Receive already-validated, tenant-scoped data from the router; they do not re-import api schemas.                                                                                                                                                                                                                                                                                                                                                                                                     |

### 10.2 Event schema consumers

| Consumer                                | Imports                                                                | Rule                                                                  |
| --------------------------------------- | ---------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `apps/api` publishers / queue consumers | payload schema + `eventEnvelopeSchema` from `@rocky/validators/events` | The event is the message-boundary contract.                           |
| Domain services / handlers              | the **payload interface** (Part 1), not Zod                            | Depend on types, not on the validator dependency.                     |
| Frontends / workers                     | **NOT** event schemas directly                                         | Events flow through the internal bus, never across the HTTP boundary. |

### 10.3 The Frontend Path to Enums

The frontend (web + expo) **never imports `domain.ts` and never imports `api/` or `events/` schemas directly.** Two supported paths, neither touching the generator file:

1. **For the TypeScript type of an enum value** — import the dictionary from the barrel `@rocky/validators/src/enums/index.js` (the `CONSTANT` re-export). This keeps the frontend off `@rocky/database` entirely (LAW XI: *"Frontend must import dictionaries from here, NOT @rocky/database"*).
2. **For a runtime array/options list** — import the per-enum dictionary module directly, e.g. `import { DRIVER_APPROVAL_STATUS } from "@rocky/database/constants/driver-approval-status"`. These are the same readonly `VALUES` the barrel re-exports, so the value set is identical.

The frontend does **not** get the branded `*Schema` validators. Those live behind the tRPC boundary: the generated client (`packages/trpc/src/generated/server.ts`) already carries the inferred types from the `api/` schemas, so a component receives a correctly-typed `status` field without ever importing a Zod schema.

**Summary:** author the shape once in `api/` (or `events/`), and it propagates to every router and every frontend client through the tRPC codegen — no manual re-typing downstream. `domain.ts` is an internal build artifact; the frontend reaches enum *values* through the dictionary modules (or the barrel) and enum *types* through the generated tRPC client.
