 ...
 * Movements are farm-scoped via from_farm_id/to_farm_id. Non-superuser
 * `rocky_rls_test` for assertions; superuser `tbot` (RLS_ADMIN_URL) for
 * scaffolding. animal_id is NOT NULL, so we reuse an existing animal id.
 */
...
const repoOn = (tx: unknown) => new MovementRepository({ client: tx } as unknown as DatabaseProvider);
...
describe.skipIf(!hasRLSEnv)("MovementRepository — RLS farm-scoped isolation (Scenario C)", () => {
  it("linked farmer sees own-farm movements; unlinked sees none", async () => {
    ...
    try {
      await db.transaction(async (tx: any) => {
        ...
        const rec = new MovementFactory(animalId, FARM_ID, FARM_ID).create({ id: M1 });
      ...
      await adminDb.execute(sql`DELETE FROM movements WHERE id = ${M1}`);
