...
export type GeoMapFeatureKind = "geofence" | "disease_zone" | "deforested" | "compliant" | (string & {});
...
function wktPolygonToRings(wkt: string): number[][][] | null {
  ...
  const rings = ringStrs
    .map((r) => r.replace(/^\(+|\s*\)+$/g, ""))
    .map((r) => r.split(/,\s*/).map((pair) => pair.trim().split(/\s+/).map(Number)))
    .filter((ring) => ring.length >= 3 && ring.every((c) => c.length === 2 && c.every((n) => Number.isFinite(n))));
...
export const GeoMapView = React.forwardRef<HTMLDivElement, GeoMapViewProps>(function GeoMapView(
  ...
  }, [styleUrl, initialViewState?.zoom, initialViewState?.longitude, initialViewState?.latitude, initialViewState]);
  ...
  React.useEffect(() => {
    ...
    const dblHandler = (e: MaplibreGl.MapMouseEvent) => {
      ...
      setDrawPts((pts) => {
        if (pts.length >= 3) {
          onPolygonDrawnRef.current?.(pts.map(([lng, lat]) => ({ latitude: lat, longitude: lng })));
        }
        return [];
      });
    };
  ...
  React.useEffect(() => {
    ...
    const data: FeatureCollection = {
      type: "FeatureCollection",
      features: [
        { type: "Feature", id: "draft", properties: {}, geometry: { type: "Polygon", coordinates: [closed] } },
      ],
    };
