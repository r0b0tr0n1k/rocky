...
typography:
  display-xl:
    fontFamily: IoskeleyMonoTerm Nerd Font
    fontSize: 38px
    fontWeight: 700
    lineHeight: 1.5
    letterSpacing: 0
...
**Key Characteristics:**
- 100% IoskeleyMonoTerm Nerd Font typography across every text role — no sans-serif fallback anywhere in the chrome
...
### Font Family
**IoskeleyMonoTerm Nerd Font** is the proprietary monospaced face used across every text role in the system. It carries weights 400 (regular), 500 (medium), and 700 (bold) and falls back through a long monospace stack — IBM Plex Mono → ui-monospace → SFMono-Regular → Menlo → Monaco → Consolas → Liberation Mono → Courier New.

The single-font decision is the brand. There is no display face, no body sans, no italic alternative, and no fallback to a proportional font anywhere — even the legal copyright row uses IoskeleyMonoTerm Nerd Font at 14px. This is the most aggressive typographic restraint of any site in the marketing-tools category: OpenCode's identity is "the marketing page is a man page."
...
IoskeleyMonoTerm Nerd Font is a paid commercial font. Open-source substitutes that approximate its metrics within ~3% at body sizes:
...
- Background `{colors.surface-card}` (`#f1eeee`), text `{colors.ink}` rendered in `{typography.body-md}` (already monospaced — IoskeleyMonoTerm Nerd Font), padding `12px 16px`, rounded `{rounded.sm}`.
...
### Do
- Render every text role in IoskeleyMonoTerm Nerd Font. The single-font decision is the entire identity.
...
### Don't
- Don't introduce a sans-serif body font, a display face, or an italic style. IoskeleyMonoTerm Nerd Font carries everything.
