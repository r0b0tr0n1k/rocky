...
export class IotRouter {
  ...
  @Query({ input: listDevicesRequestSchema })
  ...
  @Query({ input: z.object({ id: z.uuid() }) })
  ...
  @Mutation({ input: z.object({ readings: z.array(ingestReadingRequestSchema) }) })
  ...
  @Query({ input: listReadingsRequestSchema })
