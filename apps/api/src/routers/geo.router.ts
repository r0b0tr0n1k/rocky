...
export class GeoRouter {
	...
	async forwardGeocode(
		@Input()
		input: { query: string; language?: string; limit?: number; proximity?: [number, number]; bbox?: number[]; country?: string[] },
	) {
		return unwrap(
	...
	async reverseGeocode(@Input() input: { lng: number; lat: number; language?: string; limit?: number }) {
		return unwrap(await this.map.reverseGeocode([input.lng, input.lat], { language: input.language, limit: input.limit }));
	}
	...
	async staticMapUrl(@Input() input: {
	...
	async forestLayer(@Input() input: { farmId: string; cutoffYear?: number }) {
		...
		for (const g of geofences) {
			...
			const props = res.isOk()
				? { compliant: res.value.compliant, deforestedPixels: res.value.deforestedPixels, totalPixels: res.value.totalPixels }
				: {};
			features.push({
				id: g.id,
...
>;
type _verify_deleteGeofence = SubtypeGuillotine<
	void,
	Awaited<ReturnType<GeoRouter["deleteGeofence"]>>
>;
