...
/**
 * Get Bearer token from cached session.
 * Prefers token-based auth over cookies when available.
 */
export async function getAuthToken(): Promise<string | null> {
  try {
    ...
    const sessionVal = (record["session"] ?? record) as Record<string, unknown> | undefined;
    const token = sessionVal?.accessToken as string | undefined;
    return token ?? null;
  } catch {
    return null;
  }
