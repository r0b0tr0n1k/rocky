...
function collectExports(dir) {
  ...
  for (const m of src.matchAll(/export\s+(?:const|let|var|function|class|interface|type|enum)\s+([A-Za-z_$][\w$]*)/g)) names.add(m[1]);
  ...
  while ((f = fenceRe.exec(docText))) {
    ...
    while ((m = importRe.exec(block))) {
      ...
      for (const part of m[1].split(',')) {
        ...
        const name = t.replace(/^type\s+/, '').trim().split(/\s+as\s+/)[0].trim();
...
test('@rocky/validators/errors owns TRPC error maps (*_TRPC_ERROR_MAP)', () => {
  ...
  for (const file of fs.readdirSync(errDir)) {
    ...
    for (const m of src.matchAll(/export\s+const\s+(\w*_TRPC_ERROR_MAP)\b/g)) count++;
  }
...
test('functional: Result monad from @rocky/domains-shared behaves as projected', async () => {
  ...
  assert.notEqual(typeof mod.unwrap, 'function',
