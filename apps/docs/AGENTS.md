...
- `DESIGN*.md` — standardized design-system specs (`@google/design.md` format: colors/typography/rounded/spacing/components) at the repo root. Referenced from docs/how-to/ADRs by **inline-code path** (e.g. `` `DESIGN.vercel.md` ``), never as markdown links (a real link to a file outside `content/` fails `check:md-links`). They are **non-governing visual-token references** — `DESIGN.vercel.md` is harvested into `apps/docs/app/theme.css` as the secondary accent (green stays primary). See [ADR-0109](/ADR/0109-design-md-theme-workflow) + [how-to](/how-to/use-design-md-theme).
