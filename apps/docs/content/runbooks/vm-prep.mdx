...
flowchart TD
  ...
  C --> D["♻️ ./scripts/db-recreate.sh<br/>(schema + seed: Vet data + admin demo tables)"]
...
The seed includes:
- **Veterinary & Sanitary** fake data + a `lab_test.completed` outbox event (see
  [Veterinary & Sanitary Smoke Test](/runbooks/veterinary-sanitary-smoke-test));
- **Section 8 admin demo data** — `cattle_passports`, `vs_contracts`, `vs_assignments`,
  `farm_books`, `error_corrections`, `pda_devices`, `iot_devices`, `sensor_readings`,
  `notifications`, `audit_log`. These back the corresponding admin pages so they are
  populated out of the box after a fresh `db-recreate.sh`.

**Interaction-gated pages (not seeded):** `movement-lineage`, `subjects`, and `sync`
fill from user actions / require a selected region+farm filter — they are empty by
design until you drive the relevant workflow. Don't mistake them for missing data.
