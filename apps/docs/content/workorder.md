...
| WO-159 | Audit-log compliance gap-closure (ISO 27001 A.8.15 Logging + MK LPDP + EUDR traceability): (1) append-only enforcement via DB trigger on `audit_log`; (2) sensitive-field masking in `oldValue`/`newValue` snapshots (LPDP/GDPR — snapshots can carry personal data); (3) retention/partitioning automation (schema TODO — monthly partitions + prune job); (4) verify full CUD coverage — confirm every mutating path invokes `AuditService.recordUpdate` (today manual, no global interceptor). **Progress (2026-07-20):** Part 2 (PII masking in `oldValue`/`newValue`/`changes` snapshots, driven by the ADR-0061 D1 PII registry) + Remediation A (objective, non-commentary field-level diffs) implemented in `packages/domains/audit`; verified by unit tests. Part 1 (append-only DB trigger on `audit_log`, with retention-delete gated via the `app.audit_retention` GUC for WO-159 Part 3) implemented 2026-07-20 (migration `20260720093000_audit_log_append_only`; DB-backed deny test). **Part 3 (retention/partitioning) done (2026-07-20):** 7-year retention window (epidemiology law; animal-owner erasure is void — GDPR Art. 6(1)(c) legal obligation + livestock traceability exemption, so no early deletion, only age-based prune). `AuditRetentionJob` (`apps/api/src/jobs/audit-retention.job.ts`, daily `@Cron(EVERY_DAY_AT_2AM)`) computes the 7y cutoff and calls `AuditService.pruneOldRecords` → `AuditRepository.pruneRetained`, which opens a transaction, arms `app.audit_retention = 'on'` (so the Part 1 append-only trigger permits the delete), and `DELETE`s rows older than the cutoff; all rows are masked at write (Part 2), so pruning satisfies confidentiality. DB-backed prune test (`audit-log.prune.test.ts`) verifies old-row delete + recent-row retention. **Partitioning deferred:** native RANGE partitioning would force a composite PK `(id, createdAt)` that breaks global `id` uniqueness (Postgres requires the partition key in every unique constraint), and the 7y prune already meets the compliance bar — noted as a future optimization. **Part 4 (full CUD coverage) + Remediation B done (2026-07-20):** `AuditService.recordCreate`/`recordDelete` added (Remediation B threads request `ip`/`userAgent`/`traceId` from the execution CLS via `EXECUTION_CONTEXT_CLS_KEY`); wired into create+update paths across `farm`/`subject`/`animal`/`passport`/`movement`/`health`/`eartag` (animal/passport/movement/health/eartag added as non-breaking optional `auditService?` constructor param + `app.module` DI). **Both are now directly unit-tested** in `audit.service.test.ts`: CUD action mapping for `recordCreate`/`recordUpdate`/`recordDelete`, plus CLS capture of `ip`/`userAgent`/`traceId` (incl. explicit-override precedence and no-CLS fallback). **PII registry gap CLOSED:** added entries for `animals`/`cattle_passports`/`movements` + health tables (`treatments`,`vaccinations`,`lab_tests`), incl. GDPR Art.9 special-category masking (diagnosis/treatment/result/antibiotic/notes) — masking now covers all audited domains. **Out of scope:** Wazuh-vs-cloud SIEM / infra monitoring — separate infra decision, not part of the audit-log control. | 0067 / audit domain | P1 | Done ✅ |
...
| WO-163 | Inline PDF viewer (EmbedPDF) on `/documents` (verify slated) — client-side render of generated PDFs with print/download toolbar; viewing side of WO-050 | 0082 / 0084 | P2 | Done ✅ (impl + check-types + biome clean; `pnpm --filter web build` + browser verify pending) |
...
### WO-160 — ROCKY-DS 001:2026(E) §8.2 service→DB Visa-Matrix closure — P1 — Done ✅ (RED 4/4 + AMBER 6/6)

- **Constitution:** ROCKY-DS 001:2026(E) (Diamond Seal — End-to-End Type Safety — Part 1: Type Contracts) §8.2 forbids a domain service from importing `@rocky/database` (Drizzle client / table definitions / `drizzle-orm`) directly; the service reaches Postgres only through its own-domain Repository (the sole DB gatekeeper, §8.3). Cross-domain repositories may be injected in `*.module.ts` (§8.5). The Chief Architect ratified this as the repository's supreme law and vetoed the earlier Annex C Row 6 leak that listed `@rocky/database` under the service May-import column.
- **Arrears scan (2026-07-17):** of 41 domain services, 10 imported `@rocky/database` directly — a self-contradiction with the ratified standard. Classified into RED (value imports + direct query/transaction → real §8.2 breach) and AMBER (type-only `import type` / `typeof ….$inferInsert` → trivial re-export follow-up).
- **RED 4/4 remediated + committed:**
  - **A** `notification.service.ts` — deleted table imports; `NotificationRepository` re-exports `NotificationRow`/`DeviceTokenRow`; service imports them relative (`NOTIFICATION_TYPE` Dictionary constant retained — compliant). → commit `90149d9c`.
  - **B** `subscription-resolver.service.ts` — 8 direct `db.` queries routed through 5 new `NotificationRepository` methods (`findActiveSubscriptionsByEventType`/`findDeliveryByKey`/`insertDelivery`/`markDeliverySent`/`insertReminder`) + `UserRepository.findByRole` (cross-domain inject, §8.5); `db`/`drizzle-orm`/tables gone; `resolveAndNotify` converted to `Result<void,Error>` (Error Sovereignty Doctrine). → commits `e1ba0f54` + `e614da8b` (user `findByRole`).
  - **C** `risk-analysis.service.ts` — new `RiskAnalysisRepository` (riskAnalyses/riskAnalysisResults CRUD) + `FarmRepository.countActiveFarms`/`getFarmsWithRiskFactors` (3-table farm⊕animal⊕inspection aggregate); repos injected; `DatabaseProvider`/tables/`drizzle-orm` gone. The annual risk-analysis cron (`apps/api/src/jobs/risk-analysis.job.ts`) now runs inside `ExecutionPipeline.run(SYSTEM_PRINCIPAL, ctx.request, …)` so repos inherit RLS scoping (previously ran on the naked global pool). → commits `7ad0394a` + `ccd6f7a2`.
  - **D** `iot.service.ts` — deleted dead `import { iotDevices }` (never referenced). → commit `b6e62545`.
