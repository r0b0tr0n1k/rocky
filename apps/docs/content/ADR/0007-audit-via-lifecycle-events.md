...
```mermaid
flowchart LR
    ES["📡 EventsStage"] -->|"emit execution:started"| Emit["📨 ExecutionEventEmitter<br/>in-process pub/sub"]
    ES -->|"await next()"| H(["⚙️ Handler"])
    H -->|"emit execution:completed / failed"| Emit
    Emit -.->|"started"| Trace["📊 TraceSubscriber<br/>start/end span"]
    Emit -.->|"completed"| Audit["📝 AuditSubscriber<br/>record audit trail"]
    Emit -.->|"completed"| Metrics["⏱️ MetricsSubscriber<br/>record duration"]

    classDef emit fill:#87CEEB,stroke:#333,stroke-width:2px,color:darkblue
    ...
    class ES,Emit emit
    class Trace,Audit,Metrics sub
    class H io
```

_Fig. 1 — Observer pattern: `ExecutionPipeline` emits lifecycle events to an in-process `ExecutionEventEmitter`; audit, tracing, and metrics subscribe independently. The pipeline imports none of them._
