...
- **WO-118 implemented:** `validateEarTagFormat(tagNumber, format, prefix)` enforces ISO 11784/11785 (R2, USDA APHIS ADT 9 CFR 86) at animal registration. `ISO_11784_15` = exactly 15 digits prefixed by the jurisdiction ISO 3166-1 code (840=USA); `MK_8` = 8 digits with the MK weighted check digit (existing provider). Format/prefix are jurisdiction-pluggable via `RuleSet.tag` (defaults `MK_8` / `807`), seeded from `system_parameters` (no migration). Invalid tag -> `ANIMAL_INVALID_EAR_TAG` (HTTP 400 BAD_REQUEST). Entry point is `AnimalService.create` (the tag is set at registration; `createOrder` generates orders, not tags).
