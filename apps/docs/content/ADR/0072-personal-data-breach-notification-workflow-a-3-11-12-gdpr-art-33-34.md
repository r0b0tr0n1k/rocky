...
| **Related**    | ADR-0061; ADR-0067; ADR-0007; ADR-0030; compliance module |

## Context

We **log breaches after the fact but notify no one**. The Imaginary: "we log everything,
therefore we are safe" -- the Real: GDPR Art 33 demands authority notification within
**72 hours**, Art 34 demands subject notification where risk is high. Logging is evidence;
it is not notification. The crosswalk anchors ART_33 <-> MK_LPDP_ART_37 <-> AL_124_ART_28
(authority); the subject-notification leg (Art 34) is present in the standard even where
the 15-control validated subset omitted it.

## Decision

A **Breach Notification Workflow** -- on detection (audit-log anomaly or manual flag),
auto-draft the authority notification (starting the 72h clock) and, where risk is high, the
subject notification, with the tamper-evident log as the evidence pack. State machine:
`DETECTED -> ASSESSED -> NOTIFIED_AUTHORITY -> NOTIFIED_SUBJECTS -> CLOSED`.
The workflow *consumes* the audit log (ADR-0007); it does not replace it. Phase 2;
counsel confirms the authority contacts and the risk-threshold for subject notification.
...
rg -n "ADR-0067|ADR-0061" 0072-*.md
