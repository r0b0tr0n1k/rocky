...
import { GeoMapView, type GeoMapFeature, type GeoMapVertex } from "@rocky/ui/components/map";
...
export default function GeoPage() {
	...
	const [draft, setDraft] = React.useState<{ vertices: GeoMapVertex[]; name: string; fenceType: string; freeSince: string } | null>(null);

	const geofences = useQuery(trpc.geo.listGeofences.queryOptions({ farmId }, { enabled: Boolean(farmId) }));
	const diseaseZones = useQuery(
		trpc.geo.findActiveDiseaseZones.queryOptions({ farmId }, { enabled: Boolean(farmId) }),
	);
	...
	const forestLayer = useQuery(
		trpc.geo.forestLayer.queryOptions(
			{ farmId, cutoffYear: CUTOFF_YEAR },
			{ enabled: Boolean(farmId) && showForest },
		),
	);
	...
	const allGeofences = React.useMemo<GeofenceResponse[]>(() => (geofences.data ?? []) as GeofenceResponse[], [geofences.data]);
							...
							<Card>
								...
								<CardContent className="space-y-3">
									...
									{createGeofence.isError && (
										<p className="text-xs text-destructive">Could not create geofence.</p>
									)}
								</CardContent>
							</Card>
						)}
